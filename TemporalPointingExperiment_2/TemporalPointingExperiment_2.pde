...
//boolean doRandom = false;//Normal
boolean doRandom = true;///Random

Serial sp;

int numRepeat = 40;      // number of trials within a condition
int restTime = 20;      // forceful rest time between conditions
...
int rnd = 250;    // stdev of temporal distance  ==> temporal distance in experiment = distNormal(tD, rnd^2);
int speed1 = 166;  // test speed #1 (LED/s)  ````````````````=
int speed2 = 333;  // test speed #2 (LED/s)
...
class UnitExperiment
{
  ...
  public String toLogString()
  {
    ...
    ret += "d"+tDistance+",w"+tWidth+",m"+tMargin+",s"+speed+",r"+rnd;
    return ret;
  }
}